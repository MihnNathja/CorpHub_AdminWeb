...
const AbsenceRequestTable = () => {
    const { absenceRequests, setFilter, approveRequest, rejectRequest, loading, error } = useAbsenceRequest();
    const { user } = useAuth();
    ...
    const tabs = [
        ...
        { key: "approved", label: "Chấp nhận", icon: CheckCircle, color: "emerald" },
                ...
                {absenceRequests.length > 0 && (
                    <div className="px-6 py-3 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
                        <p className="text-xs font-medium text-gray-600 dark:text-gray-400">
                            Showing <span className="font-semibold text-gray-900 dark:text-gray-100">{absenceRequests.length}</span> request{absenceRequests.length !== 1 ? "s" : ""}
